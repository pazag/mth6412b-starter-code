...
function read_header5(filename::String)
...
function read_nodes5(header::Dict{String}{String}, filename::String)
...
function n_nodes_to_read5(format::String, n::Int, dim::Int)
...
function read_edges5(header::Dict{String}{String}, filename::String)
    ...
    n_edges = 0
    i = 0
    n_to_read = n_nodes_to_read5(edge_weight_format, k, dim)
    flag = false
...
function read_stsp5(filename::String)
    Base.print("Reading of header : ")
    header = read_header5(filename)
    println("✓")
    dim = parse(Int, header["DIMENSION"])
    edge_weight_format = header["EDGE_WEIGHT_FORMAT"]

    Base.print("Reading of nodes : ")
    graph_nodes = read_nodes5(header, filename)
    println("✓")

    Base.print("Reading of edges : ")
    edges_brut = read_edges5(header, filename)
    graph_edges = []
    for k = 1 : dim
        edge_list = Int[]
...
Exemple :
        ...
        savefig("bayg29.pdf")
"""
function plot_graph5(nodes, edges)
...
function plot_graph5(filename::String)
    graph_nodes, graph_edges = read_stsp5(filename)
    plot_graph5(graph_nodes, graph_edges)
end
